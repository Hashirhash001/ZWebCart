"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FrontStoreGuard = void 0;
const common_1 = require("@nestjs/common");
const prisma_central_service_1 = require("../../prisma/prisma-central.service");
const crypto_utils_1 = require("../../central/utils/crypto-utils");
let FrontStoreGuard = class FrontStoreGuard {
    constructor(prismaCentral) {
        this.prismaCentral = prismaCentral;
    }
    async canActivate(context) {
        const request = context.switchToHttp().getRequest();
        const storeId = request.body?.storeId;
        if (!storeId || typeof storeId !== 'string') {
            throw new common_1.ForbiddenException('Invalid or missing store ID in the request body.');
        }
        const store = await this.prismaCentral.store.findUnique({
            where: { id: String(storeId) },
        });
        if (!store) {
            throw new common_1.ForbiddenException('Store not found.');
        }
        const decryptedDbUrl = this.decryptDatabaseUrl(store.dbUrl);
        request['storeDbUrl'] = decryptedDbUrl;
        request['storeId'] = storeId;
        return true;
    }
    decryptDatabaseUrl(dbUrl) {
        try {
            return (0, crypto_utils_1.decrypt)(dbUrl);
        }
        catch (error) {
            console.error('Database URL Decryption Failed:', error);
            throw new common_1.ForbiddenException('Failed to decrypt database URL.');
        }
    }
};
exports.FrontStoreGuard = FrontStoreGuard;
exports.FrontStoreGuard = FrontStoreGuard = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_central_service_1.PrismaCentralService])
], FrontStoreGuard);
//# sourceMappingURL=frontStore.guard.js.map